...
* **内部可変性** は、そのデータへの不変参照がある時でさえもデータを可変化できる Rust のデザインパターン
* 通常、そのような操作は借用規則により許可されないが、このパターンではデータ構造内で `unsafe` を使用して、通常の借用規則を捻じ曲げる
* コンパイラでは保証できなくても、実行時に借用規則に従うことが保証できる時、内部可変性パターンを使用する
...
* Rust のメモリ安全保障によりリークはしにくいが、不可能ではない
* `Rc` と `RefCell` で循環参照を作ると、各要素のカウントが絶対に 0 にならないので、メモリリークを起こし値は絶対にドロップされない
...
上記コードは以下のような関係になってしまう

![image](https://user-images.githubusercontent.com/1415655/106382832-7aa24100-6405-11eb-8b41-9df6b783c3d2.png)


# 恐れるな！並行性

所有権と型チェックを活用することで、多くの並行性に関する実行時エラーはコンパイルエラーにすることができる。

## スレッドを使用してコードを同時に走らせる

マルチスレッドのコードは以下のような問題を招く

* レースコンディション
* デッドロック
* 特定の状況でのみ発生し、再現や修正が困難なバグ

Rust は、所有権と型チェックによりこれらの悪影響を軽減しようとしているが、それでもマルチスレッドプログラミングには注意深い作業が必要となる。

Rust のスレッドモデルは、OS スレッドと紐づく 1:1 モデル。
...
* チャンネルを使った情報共有は単独の所有権に類似している
* メモリ共有の並行性は複数の所有権に類似している

### ミューテックスを使用して一度に1つのスレッドからデータにアクセスすることを許可する

Rust の所有権と型システムにより、ミューテックスを比較的安全に使うサポートを得ることができる。
以下はまず単独スレッドの例。
...
fn main() {
    ...
    {
        // lock() はミューテックスのロックを得られるまで現在のスレッドをブロックする
        // ロックを保持している他スレッドがパニックしたら lock の呼び出しは失敗し、誰もロックを取得することはできなくなる。
        // ここでは unwrap し、そのような状況になったらパニックするようにしている。
        let mut num = m.lock().unwrap();
        *num = 6;

        // lock の呼び出しは、スマートポインタの MutexGuard を返却する。
        // MutexGuard には、内部のデータを指す Deref メソッドと、スコープから外れた場合にロックを解除する `Drop` メソッドが実装されている。
    }
...
Sync と Send トレイトを実装することで、独自の並行性機能を実装することができる。

### Send でスレッド間の所有権の転送を許可する

`Send` マーカートレイトは、その所有権をスレッド間で転送できることを示す。

Rust のほとんどの型は `Send` だが、`Rc<T>` など一部例外がある。
`Rc` は `Send` を実装しないので、その所有権を別スレッドに転送しようとするとコンパイルエラーとなる。
一方、`Arc` は `Send` を実装するので、所有権を別スレッドに転送することができる。

### Sync で複数のスレッドからのアクセスを許可する

`Sync` マーカートレイトは、その参照が複数のスレッドからアクセスされても安全であることを示す。
言い換えると、`&T` が `Send` であるとき、その `T` は `Sync` であると言え、参照が他のスレッドに安全に送信できることを示す。

* スマートポインタの `Rc` は `Sync` ではない
* `RefCell` と関連する `Cell` 型もまた `Sync` ではない
* `Mutex` は `Sync` なので、複数のスレッドでアクセスを共有するのに使用することができる

### Send と Sync を手動で実装するのは非安全である

SendとSyncトレイトから構成される型は自動的にSendとSyncにもなるので、 それらのトレイトを手動で実装する必要はありません。マーカートレイトとして、 実装すべきメソッドさえも何もありません。並行性に関連する不変条件を強制することに役立つだけなのです。

これらのトレイトを手動で実装するには、unsafeなRustコードを実装することが関わってきます。 unsafeなRustコードを使用することについては第19章で語ります; とりあえず、重要な情報は、 SendとSyncではない部品からなる新しい並行な型を構成するには、安全性保証を保持するために、 注意深い思考が必要になるということです。The Rustonomiconには、 これらの保証とそれを保持する方法についての情報がより多くあります。
...
GoF によると OOP とは以下であると定義されている。

> オブジェクト指向プログラムは、オブジェクトで構成される。オブジェクトは、 データとそのデータを処理するプロシージャを梱包している。このプロシージャは、 典型的にメソッドまたはオペレーションと呼ばれる。

この定義によると Rust もオブジェクト指向であると言える。構造体と `enum` がデータをもち、`impl` が構造体と `enum`　にメソッドを提供する。
Rust の中では、オブジェクトという用語は登場しないものの、GoF の OOP の定義と同じ機能を提供する。

### カプセル化は実装詳細を隠蔽する

Rust は `pub` を使い、公開 API と非公開 API を制御することができるので、カプセル化可能といえる

### 型システム、及び共有コードとしての継承

Rust には継承はないが、トレイトを通してコードの共有を行うことができる
